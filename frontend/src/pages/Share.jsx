...
function ProducerView({ data, hideGear, onOpenShotList }) {
  ...
  const drives = isLocalPerson ? [] : allDrives;
...
function ShareGlassDock({ items }) {
  ...
  return (
    <div className="share-dock no-print" style={{
      ...
      zIndex:110, display:'flex', alignItems:'center', gap:2, maxWidth:'calc(100vw - 72px)', overflowX:'auto',
...
function DaySection({ day, showCalls, flights, drives, dayIndex, talentCallTime, talentCallLocation, hideCallWrap, tagFilter, personFilter, cateringDetail, shotList, slDays, slBreaks, onOpenShotList, crewAssignments, projectCity, talentMode, includePhoto }) {
  ...
  const cateringItems = (tagFilter || talentMode) ? [] : (day.catering || [])
    ...
    .map(c => ({ _type:'catering', _sort: c.delivery_time ? timeToMins(c.delivery_time) : (c.meal_type === 'BREAKFAST' ? -1 : c.meal_type === 'DINNER' ? 99999 : 9997), _key:`cat-${c.id}`, ...c }));
                ...
                return (
                  <div key={item._key} className="ev">
                    ...
                    <div className="ev-body" style={{ borderLeft:'2px solid #4a9eff' }}>
                      ...
                      {(item.origin || item.destination) && <div className="ev-detail">{item.origin || '?'} → {item.destination || '?'}</div>}
                    </div>
                  </div>
                );
              })() : item._type === 'slbreak' ? (() => {
                  ...
                  <div key={item._key} className="ev" style={onOpenShotList ? { cursor:'pointer' } : undefined} onClick={() => onOpenShotList?.()}>
                    ...
                    <div className="ev-body" style={{ borderLeft:`2px solid ${st.border}`, padding:'10px 14px' }}>
                      <div style={{ minWidth:0 }}>
                        ...
                        <div style={{ display:'flex', alignItems:'baseline', gap:10, marginTop:3, minWidth:0 }}>
                          {item.description
                            ? <div style={{ fontSize:11, color:'var(--muted)', flex:'1 1 auto', minWidth:0, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{item.description}</div>
