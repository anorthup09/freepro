...
function ContractDetailModal({ row, contract, loading, onClose, onCrew }) {
  const c = contract;
  const money = c
      ...
      <div onClick={e => e.stopPropagation()} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 12, width: '100%', maxWidth: 520, maxHeight: '90vh', overflowY: 'auto', padding: 22 }}>
        ...
        <div style={{ marginTop: 6 }}>
          {c
            ...
            : <span style={{ background: 'rgba(224,82,82,0.12)', border: '1px solid #e05252', color: '#e05252', borderRadius: 12, padding: '2px 10px', fontSize: 10, fontWeight: 800 }}>No contract sent yet — showing crew-slot estimate</span>}
        </div>
        ...
        {!loading && (
          <>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 16 }}>
              ...
              <div><div style={dt}>Labor</div><div style={dv}>{fmt$(money.dayRate)} × {Number(money.laborDays || 0)} day{Number(money.laborDays || 0) !== 1 ? 's' : ''}</div><div style={{ fontSize: 10, color: '#5ABF80', fontWeight: 700 }}>{fmt$(laborTotal)}</div></div>
            ...
            <div style={{ marginTop: 14, padding: '10px 12px', background: 'rgba(90,191,128,0.08)', border: '1px solid rgba(90,191,128,0.35)', borderRadius: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
              ...
              <span style={{ fontSize: 16, fontWeight: 800, color: '#5ABF80' }}>{fmt$(laborTotal + gearTotal)}</span>
            </div>
            {c?.scope && (
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <div style={{ maxWidth: 980, margin: '0 auto', padding: '10px 16px 60px' }}>
        ...
        <div style={{ fontSize: 10, color: 'var(--muted)', marginTop: 8 }}>
          Est. Total = day rate × labor days + gear rate × gear days from the crew slot. Click a row to see the contract details.
        </div>
        {detail && (
