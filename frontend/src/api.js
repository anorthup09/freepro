...
export const api = {
  ...
  debriefPrograms: (client) => req('GET', `/debrief/programs${client ? `?client=${encodeURIComponent(client)}` : ''}`),
  ...
  deleteMediaStorageRequest: (id) => req('DELETE', `/media-storage/${id}`),
  ...
  duplicateAvoEdit: (id) => req('POST', `/avo/edits/${id}/duplicate`),
  holdEditCost: (id, role) => req('POST', `/avo/edits/${id}/hold-cost`, { role: role || 'editor' }),
  avoEditContract: (id) => req('POST', `/avo/edits/${id}/contract`),
  recordV1Approval: (editId, recipientMemberId) => req('POST', `/avo/edits/${editId}/v1-approval`, { recipientMemberId }),
  getCelebrations: () => req('GET', '/dashboard/celebrations'),
  markAnnouncementSeen: (id) => req('POST', `/dashboard/celebrations/${id}/seen`),
  celebrateApproval: (id) => req('POST', `/dashboard/celebrations/${id}/celebrate`),
  markDropsSeen: (ids) => req('POST', '/dashboard/celebrations/drops/seen', { ids }),
  ...
  avoComment: (id, body, files) => req('POST', `/avo/edits/${id}/comments`, { body, files }),
  avoCreateFolder: (pageId, name) => req('POST', `/avo/projects/${pageId}/folders`, { name }),
  avoRenameFolder: (fid, name) => req('PATCH', `/avo/folders/${fid}`, { name }),
  avoDeleteFolder: (fid) => req('DELETE', `/avo/folders/${fid}`),
  deleteAvoComment: (id, aid) => req('DELETE', `/avo/edits/${id}/comments/${aid}`),
  avoRfr: (id, notes) => req('POST', `/avo/edits/${id}/rfr`, { notes }),
  ...
  allFunFacts: () => req('GET', '/dashboard/funfact/all'),
  deleteFunFact: (id) => req('DELETE', `/dashboard/funfact/${id}`),
  ...
  miscEvents: () => req('GET', '/team/events'),
  createMiscEvent: (data) => req('POST', '/team/events', data),
  updateMiscEvent: (id, data) => req('PATCH', `/team/events/${id}`, data),
  deleteMiscEvent: (id) => req('DELETE', `/team/events/${id}`),
  ...
  avoShareApi: (token, pw) => {
    ...
    return {
      ...
      duplicateAvoEdit: (id) => R('POST', `/edits/${id}/duplicate`),
      avoEdit: (id) => R('GET', `/edits/${id}`),
      avoRfr: (id, notes) => R('POST', `/edits/${id}/rfr`, { notes }),
      avoSent: (id) => R('POST', `/edits/${id}/sent`, {}),
      avoComment: (id, body) => R('POST', `/edits/${id}/comments`, { body }),
  ...
  addProjectLink: (pid, data) => req('POST', `/project-overview/${pid}/links`, data),
  deleteProjectLink: (id) => req('DELETE', `/project-overview/links/${id}`),
  ...
  budgetVersionSummary: (pid) => req('GET', `/finance/${pid}/version-summary`),
  markBudgetOpened: (bid) => req('POST', `/finance/budget/${bid}/opened`),
  ...
  addEventAttachment: (projectId, eventId, data) => req('POST', `/projects/${projectId}/schedule/events/${eventId}/attachments`, data),
  deleteEventAttachment: (attId) => req('DELETE', `/projects/schedule/attachments/${attId}`),
  eventAttachmentUrl: (attId, inline) => `/api/projects/schedule/attachments/${attId}/file${inline ? '?inline=1' : ''}`,
  ...
  updateDrive: (projectId, id, data) => req('PUT', `/projects/${projectId}/travel/drives/${id}`, data),
  ...
  // Shot List custom columns (shared across the whole shot list)
  getShotListColumns: (projectId) => req('GET', `/projects/${projectId}/shot-list/columns`),
