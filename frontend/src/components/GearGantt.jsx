...
export default function GearGantt({ rows, onOpen }) {
  ...
  return (
    <div ref={scrollRef} className="glass" style={{ borderRadius:14, overflowX:'auto', overflowY:'auto', maxHeight: 24 + 34 + 5 * 40 + 2, marginBottom:16 }}>
      <div style={{ width: G_NAME_W + totalDays * G_DAY_W, position:'relative' }}>
        {/* month row */}
        <div className="cal-name" style={{ display:'flex', height:24, borderBottom:'1px solid rgba(255,255,255,0.04)', position:'sticky', top:0, zIndex:4 }}>
          <div className="cal-name" style={{ width:G_NAME_W, flexShrink:0, position:'sticky', left:0, zIndex:3, borderRight:'1px solid var(--border)' }} />
        ...
        {/* day header */}
        <div className="cal-name" style={{ display:'flex', borderBottom:'1px solid var(--border)', position:'sticky', top:24, zIndex:4 }}>
          <div className="cal-name" style={{ width:G_NAME_W, flexShrink:0, position:'sticky', left:0, zIndex:3, borderRight:'1px solid var(--border)' }} />
        ...
        {sorted.map(r => {
          ...
          return (
            <div key={r.id} style={{ display:'flex', borderBottom:'1px solid rgba(255,255,255,0.04)', position:'relative', minHeight:40 }}>
              <div className="cal-name" style={{ width:G_NAME_W, flexShrink:0, padding:'10px 12px', fontSize:12, fontWeight:700, position:'sticky', left:0, zIndex:2, borderRight:'1px solid var(--border)', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{title}</div>
              {Array.from({ length: totalDays }, (_, i) => {
              ...
              <div className="cal-bar" onClick={() => onOpen && onOpen(r.id)} title={`${title} · ${fmtGD(r.start_date)} – ${fmtGD(r.end_date || r.start_date)}`}
                style={{ position:'absolute', top:8, height:24, zIndex:1, left:G_NAME_W + from * G_DAY_W, width:(to - from + 1) * G_DAY_W - 4,
                  backgroundColor:'rgba(232,80,10,0.2)', border:'1px solid var(--orange)', borderRadius:6, display:'flex', alignItems:'center', padding:'0 8px', overflow:'hidden', cursor:'pointer', fontSize:10, fontWeight:800, color:'var(--orange)', whiteSpace:'nowrap' }}>
                {title}
              </div>
            </div>
          );
