...
router.get('/:token', async (req, res, next) => {
  try {
    ...
    const [project] = await sql`
      SELECT p.id, p.code, p.title, p.subtitle, p.client, p.city, p.state, p.start_date, p.end_date, p.status, p.notes, p.share_password, p.show_shot_list, p.show_scripts, p.client_logo, p.include_photo, p.schedule_tag_colors,
    ...
    const daysWithData = await Promise.all(shootDays.map(async day => {
      ...
      return {
        ...
        events: events
          ...
          .map(e => ({ ...e, tags: e.tags || [], crew_ids: e.crew_ids || [], location: e.location_name ? { name: e.location_name, address: e.location_address } : (e.adhoc_location ? { name: e.adhoc_location, address: e.adhoc_address || null, adhoc: true } : null) })),
        crewCalls: crewCalls
    ...
    if (viewType === 'producer') {
      ...
      const [flights, hotelBlocks, rentalCars, deliverables, gear, onlineRentals, shotListScenes, shotListDays, shotListBreaks, drives] = await Promise.all([
        ...
        safe(sql`SELECT rc.id, rc.vendor, rc.pickup_location, rc.dropoff_location, rc.pickup_date, rc.dropoff_date, rc.confirmation, rc.notes, COALESCE(NULLIF(TRIM(CONCAT(cm.preferred_first_name, ' ', cm.preferred_last_name)), ''), cm.name) as crew_name FROM rental_cars rc LEFT JOIN crew_members cm ON cm.id = rc.crew_member_id WHERE rc.project_id = ${projectId}`),
        safe(sql`SELECT d.id, d.title, d.description, d.status, d.editor_name, d.aspect_ratio, d.resolution, d.due_date, d.is_urgent, d.category, e.tracker_type FROM deliverables d LEFT JOIN edits e ON e.deliverable_id = d.id WHERE d.project_id = ${projectId} AND e.archived IS NOT TRUE ORDER BY CASE d.category WHEN 'PRE_PRODUCED' THEN 0 WHEN 'ON_SITE' THEN 1 ELSE 2 END, d.created_at`),
        safe(sql`SELECT pg.*, COALESCE(NULLIF(TRIM(CONCAT(cm.preferred_first_name, ' ', cm.preferred_last_name)), ''), cm.name) as gear_person_name, cm.phone as gear_person_phone, cm.email as gear_person_email FROM project_gear pg LEFT JOIN crew_members cm ON cm.id = pg.gear_person_id WHERE pg.project_id = ${projectId}`),
      ...
      const [crewFlights, crewHotels, crewCars, crewDeliverables, crewGear, crewOnlineRentals, crewShotList, crewSlDays, crewSlBreaks, crewDrives] = await Promise.all([
        ...
        safe2(sql`SELECT hb.id, hb.name, hb.address, hb.phone,
            ...
            FROM hotel_blocks hb LEFT JOIN hotel_guests hg ON hg.hotel_block_id = hb.id
            WHERE hb.project_id = ${projectId} GROUP BY hb.id, hb.name, hb.address, hb.phone`),
        safe2(sql`SELECT rc.id, rc.vendor, rc.pickup_location, rc.dropoff_location, rc.pickup_date, rc.dropoff_date, rc.confirmation, rc.notes, COALESCE(NULLIF(TRIM(CONCAT(cm.preferred_first_name, ' ', cm.preferred_last_name)), ''), cm.name) as crew_name FROM rental_cars rc LEFT JOIN crew_members cm ON cm.id = rc.crew_member_id WHERE rc.project_id = ${projectId}`),
        safe2(sql`SELECT d.id, d.title, d.description, d.status, d.editor_name, d.aspect_ratio, d.resolution, d.due_date, d.is_urgent, d.category, e.tracker_type FROM deliverables d LEFT JOIN edits e ON e.deliverable_id = d.id WHERE d.project_id = ${projectId} AND e.archived IS NOT TRUE ORDER BY CASE d.category WHEN 'PRE_PRODUCED' THEN 0 WHEN 'ON_SITE' THEN 1 ELSE 2 END, d.created_at`),
        safe2(sql`SELECT pg.*, COALESCE(NULLIF(TRIM(CONCAT(cm.preferred_first_name, ' ', cm.preferred_last_name)), ''), cm.name) as gear_person_name, cm.phone as gear_person_phone, cm.email as gear_person_email FROM project_gear pg LEFT JOIN crew_members cm ON cm.id = pg.gear_person_id WHERE pg.project_id = ${projectId}`),
      ...
      const [clientShotList, clientSlDays] = await Promise.all([
        ...
        safeC(sql`SELECT * FROM shot_list_days WHERE project_id = ${projectId} ORDER BY sort_order, day_number`),
      ]);
      responseData = {
        ...responseData,
        locations,
      ...
      const talentDayCalls = talentRecord
        ? await sql`SELECT shoot_day_id, call_time, call_location FROM talent_day_calls WHERE talent_id = ${talentRecord.id}`
        : [];
