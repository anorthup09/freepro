...
async function migrate() {
  ...
  await sql`
    DO $$ BEGIN
      ...
      IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'event_tag_type') THEN
        CREATE TYPE event_tag_type AS ENUM ('VIDEO','PHOTO','AUDIO','ALL_CREW','TALENT','TRAVEL','CUSTOM','GENERAL');
      END IF;
    END $$
  `;
  ...
  // Talent are added without a role/title in the add form; role is optional
  await sql`ALTER TABLE key_talent ALTER COLUMN role DROP NOT NULL`;
  ...
  // Per-row color-coding on the Project Video Tracker (hex, null = none)
  ...
  await sql`ALTER TABLE deliverables ADD COLUMN IF NOT EXISTS frame_rate TEXT`;
  ...
  await sql`ALTER TYPE event_tag_type ADD VALUE IF NOT EXISTS 'GENERAL'`;
  ...
  // One-time: undo the earlier single-production "rename in place". The overall
  // project (parent) code must stay bare (02.SCC90126) — only the budget's
  // Production Costs sections carry a suffix (…-01, -02), and FreePro views render
  // that suffix through displayCodes(). An earlier migration wrongly renamed the
  // parent project code to the shoot_code; strip that suffix back off and revert
  // the string keys it cascaded to (edits + AvocadoPost pages). Only touches
  // top-level projects that are their own single-production tile — never real
  // multi-shoot child projects (those legitimately keep their suffixed code).
  try {
    const revertCandidates = await sql`
      SELECT p.id AS pid, p.code AS oldcode, regexp_replace(p.code, '-[0-9]+$', '') AS basecode
      FROM projects p
      JOIN budget_sections bs ON bs.freepro_project_id = p.id AND bs.kind = 'shoot'
      JOIN budgets b ON b.id = bs.budget_id AND COALESCE(b.kind, 'main') = 'main' AND b.project_id = p.id
      WHERE p.parent_project_id IS NULL
        AND p.code ~ '-[0-9]+$'
        AND bs.shoot_code = p.code`;
    for (const c of revertCandidates) {
      if (c.basecode === c.oldcode) continue;
      const [clash] = await sql`SELECT id FROM projects WHERE code = ${c.basecode} AND id <> ${c.pid}`;
      if (clash) continue;
      await sql`UPDATE projects SET code = ${c.basecode} WHERE id = ${c.pid}`;
      await sql`UPDATE edits SET project_code = ${c.basecode} WHERE project_code = ${c.oldcode}`;
      await sql`UPDATE avo_project_pages SET code = ${c.basecode} WHERE code = ${c.oldcode}`;
    }
